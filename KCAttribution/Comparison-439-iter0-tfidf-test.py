...
train_subjects = np.load("label_train_val_subjects.npy", allow_pickle=True)
test_subjects = np.load("label_test_subjects.npy", allow_pickle=True)
...
for i in range(resample_times):
    ...
    for student in range(len(train_run_label)):
        for prob_ind in range(len(train_run_problem[student])):
            if train_run_problem[student][prob_ind] in practiced_problems and train_run_score[student][prob_ind] == 0: 
