...
# test_subjects = np.load("label_test_subjects.npy", allow_pickle=True) # No need in val
    ...
    def forward(self, pred, labels, scores, problems, practicing_problems, first):
        ...
        for student in range(pred.shape[0]):
            ...
            for prob_ind in range(len(scores[student])):
                ...
                if problems[student][prob_ind] in practicing_problems and scores[student][prob_ind] == 0: # Finetune on incorrect labeled submissions
...
for i, (train_index, val_index) in enumerate(kf.split(train_val_subjects)):
    ...
    val_label, val_data, val_problem, val_score, val_first = read_seq_df(val_df, val_subjects, KC_index)
...
#     val_converted_data += train_converted_data
    ...
    for epoch in range(max_epochs):
        ...
        for _, batch_index in mini_batches:
            ...
            loss, binary_preds, ground_truths = loss_func(predictions, batch_labels, batch_scores, batch_problem, practiced_problems, batch_first_attempt)
