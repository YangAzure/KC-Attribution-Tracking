...
class lossFunc(nn.Module):
    ...
    def forward(self, pred, labels, scores, problems, practicing_problems, first, test=False):
        ...
        for student in range(pred.shape[0]):
            ...
            for prob_ind in range(len(scores[student])):
                if not test:
                    if problems[student][prob_ind] in practicing_problems and scores[student][prob_ind] == 0: # The submission is correct, but not necessary practiced.
...
#     val_converted_data += train_converted_data
    ...
    for _, test_batch_index in test_mini_batches:
        ...
        loss, binary_preds, ground_truths = loss_func(predictions, batch_labels, batch_scores, batch_problem, practiced_problems, batch_first_attempt, test=True)
#         print(ground_truths)
