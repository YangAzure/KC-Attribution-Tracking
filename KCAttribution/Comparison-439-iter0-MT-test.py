...
train_subjects = np.load("KFold_Splits/48/label_train_val_subjects_4.npy", allow_pickle=True)
...
unlabeled_subjects = pd.unique(processed_df[~processed_df['SubjectID'].isin(list(train_subjects)+list(test_subjects))]['SubjectID'])
...
for i in range(resample_times):
    ...
    test_label, test_data, test_problem, test_score, test_first, test_labeled = read_seq_df(test_df, test_subjects, KC_index, labeled=True)
...
#     val_converted_data += train_converted_data
    ...
    for epoch in range(max_epochs):
        ...
        mini_batches = model.get_minibatches_idx(train_converted_data, min(batch_size,len(train_converted_data)), shuffle=True, droplast=True)
        epoch_loss = 0
    ...
    for _, test_batch_index in test_mini_batches:
        ...
        batch_labeled = torch.nn.utils.rnn.pad_sequence([torch.tensor(test_labeled[t]) for t in test_batch_index], batch_first=True, padding_value=0)
