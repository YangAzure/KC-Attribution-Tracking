...
train_subjects = np.load("KFold_Splits/48/label_train_val_subjects_0.npy", allow_pickle=True)
test_subjects = np.load("KFold_Splits/48/label_test_subjects_0.npy", allow_pickle=True)
...
class lossFunc(nn.Module):
    ...
    def forward(self, pred, labels, scores, problems, practicing_problems, first, code, test=False):
        ...
        return loss, binary_preds, ground_truths, code_list
...
pd.DataFrame(data={"code":code_used,
                   ...
                   "pred_std":pred_std,
                   "pred_err":pred_err,
                   "GT":ground_truths.detach().numpy()}).to_csv("label_case_" +str(KC_index)+ "_0.csv")
