...
train_val_subjects = np.load("KFold_Splits/48/label_train_val_subjects_4.npy", allow_pickle=True)
test_subjects = np.load("KFold_Splits/48/label_test_subjects_4.npy", allow_pickle=True)
...
def read_seq_df(df, subjects, KC_index, labeled):
    """
    ...
    for s in subjects:
        ...
        for index, row in student_df.iterrows():
            if type(row['RawASTPath']) == str:
                ...
                s_separated_paths_labeled.append(labeled)
        ...
        separated_paths_labeled.append(s_separated_paths_labeled)


    return separated_paths_label, separated_paths_data, separated_paths_problem, separated_paths_score, separated_paths_first, separated_paths_labeled
...
class KCAttributorLC(nn.Module):
    ...
    def __init__(self, node_count, path_count, P_number):
        ...
        self.fc_correct = nn.Linear(300, 1)
    ...
    def forward(self, starting_node_index, ending_node_index, path_index, evaluating=False):
        ...
        out = self.sig(self.fc_skill(code_vectors)) # KC relavant pred
        out2 = self.sig(self.fc_correct(code_vectors)) # KC correctness pred
        
        return out, out2, code_vectors
...
class lossFunc(nn.Module):
    ...
    def forward(self, k_pred, c_pred, labels, scores, problems, practicing_problems, first, labeled):
        ...
        k_ground_truths = torch.tensor([])
        c_ground_truths = torch.tensor([])
        k_pred = k_pred.to('cpu')
        c_pred = c_pred.to('cpu')

        for student in range(k_pred.shape[0]):
            
            for prob_ind in range(len(scores[student])):
                

                if scores[student][prob_ind] == 1: # correct submission
                    if problems[student][prob_ind] in practicing_problems: # KC relavant
                        # Task 1
                        k_preds = torch.cat([k_preds, k_pred[student][prob_ind]]) 
                        k_ground_truths = torch.cat([k_ground_truths, torch.tensor([labels[student][prob_ind]])])
                        k_detect_sample += 1

#                         # Task 2 
#                         # May include or not, default is exclude, which can be decoupled as two models.
#                         # Warning: Need to reimplement the cat part for test if include.
#                         c_binary_preds = torch.cat([c_binary_preds, c_pred[student][prob_ind]]) 
#                         c_ground_truths = torch.cat([c_ground_truths, torch.tensor([c_labels[student][prob_ind]])])
#                         c_detect_sample += 1
                    else: # KC irrelavant
                        # Task 1
                        k_preds = torch.cat([k_preds, k_pred[student][prob_ind]]) 
                        ...
                        # Task 2 ignored
                elif labeled[student][prob_ind]: # labeled incorrect submission
                    if problems[student][prob_ind] in practicing_problems: # KC relavant
                        # Task 2 
                        c_preds = torch.cat([c_preds, c_pred[student][prob_ind]]) 
                        c_ground_truths = torch.cat([c_ground_truths, torch.tensor([labels[student][prob_ind]])])
                        c_detect_sample += 1
                        

        k_loss = self.crossEntropy(k_preds, k_ground_truths)/k_detect_sample
        c_loss = self.crossEntropy(c_preds, c_ground_truths)/c_detect_sample

        loss = k_loss + c_loss
        
                
        return loss, c_preds, c_ground_truths
...
unlabeled_subjects = pd.unique(processed_df[~processed_df['SubjectID'].isin(list(train_val_subjects)+list(test_subjects))]['SubjectID'])

for i, (train_index, val_index) in enumerate(kf.split(train_val_subjects)):
    
    # Train val split on correct samples
    train_subjects, val_subjects = train_val_subjects[train_index], train_val_subjects[val_index]
    ...
    val_df = original_processed_df[original_processed_df['SubjectID'].isin(val_subjects)]
    train_df = train_df[train_df['CodeStateID'].isin(label_df['CodeStateID'])]
    val_df = val_df[val_df['CodeStateID'].isin(label_df['CodeStateID'])]
    
    unlabeled_df = original_processed_df[original_processed_df['SubjectID'].isin(unlabeled_subjects)]
    
    
    for c in tracking_df.columns:
        train_df[c] = [1-label_df[label_df['CodeStateID'] == x[1]['CodeStateID']][c].iloc[0] for x in train_df.iterrows()]
        val_df[c] = [1-label_df[label_df['CodeStateID'] == x[1]['CodeStateID']][c].iloc[0] for x in val_df.iterrows()] # Readin labels
        unlabeled_df[c] = [tracking_df.loc[x[1]['ProblemID'], c] for x in unlabeled_df.iterrows()]

    train_run_label, train_run_data, train_run_problem, train_run_score, train_run_first, train_run_labeled = read_seq_df(train_df, train_subjects, KC_index, labeled=True)
    val_label, val_data, val_problem, val_score, val_first, val_labeled = read_seq_df(val_df, val_subjects, KC_index, labeled=True)
    
    a_train_run_label, a_train_run_data, a_train_run_problem, a_train_run_score, a_train_run_first, a_train_run_labeled = read_seq_df(unlabeled_df, unlabeled_subjects, KC_index, labeled=False)

    train_run_label += a_train_run_label
    train_run_data += a_train_run_data
    train_run_problem += a_train_run_problem
    train_run_score += a_train_run_score
    train_run_first += a_train_run_first
    train_run_labeled += a_train_run_labeled
    ...
    val_converted_data = [[convert_to_idx(sample, node_word_index, path_word_index) for sample in student_sample] for student_sample  in val_data]
...
#     val_converted_data += train_converted_data
    ...
    for epoch in range(max_epochs):
        model.train()
        mini_batches = model.get_minibatches_idx(train_converted_data, min(batch_size,len(train_converted_data)), shuffle=True)
        epoch_loss = 0
        for _, batch_index in mini_batches:
            ...
            batch_labeled = torch.nn.utils.rnn.pad_sequence([torch.tensor(train_run_labeled[t]) for t in batch_index], batch_first=True, padding_value=0)
            
            k_predictions, c_predictions, code_vectors = model(starting_node_index,ending_node_index,path_index)

            loss, binary_preds, ground_truths = loss_func(k_predictions, c_predictions, batch_labels, batch_scores, batch_problem, practiced_problems, batch_first_attempt, batch_labeled)
    ...
    #validation
    model.eval()
    val_mini_batches = model.get_minibatches_idx(val_converted_data, min(batch_size,len(val_converted_data)), shuffle=False)
    val_epoch_loss = 0
    val_epoch_lc_loss = 0
    for _, val_batch_index in val_mini_batches:
        batch_samples = [val_converted_data[t] for t in val_batch_index]
        raw_starting_node_index = [[[p[0] for p in single_sample] for single_sample in student_samples] for student_samples in batch_samples]
        raw_ending_node_index = [[[p[2] for p in single_sample] for single_sample in student_samples] for student_samples in batch_samples]
        raw_path_index = [[[p[1] for p in single_sample] for single_sample in student_samples] for student_samples in batch_samples]
        batch_max_len = max(len(student_samples) for student_samples in batch_samples)
        ...
        batch_labels = [val_label[t] for t in val_batch_index]

        batch_scores = [val_score[t] for t in val_batch_index]

        batch_problem = torch.nn.utils.rnn.pad_sequence([torch.tensor(val_problem[t]) for t in val_batch_index], batch_first=True, padding_value=0)
        batch_first_attempt = torch.nn.utils.rnn.pad_sequence([torch.tensor(val_first[t]) for t in val_batch_index], batch_first=True, padding_value=0)
        batch_labeled = torch.nn.utils.rnn.pad_sequence([torch.tensor(val_labeled[t]) for t in val_batch_index], batch_first=True, padding_value=0)
...
#             print(binary_preds, ground_truths)
        ...
        val_epoch_loss += loss.item()*len(val_batch_index)
        

    # Calculating the prediction scores and the classification results.
    val_loss = val_epoch_loss / len(val_converted_data)
#     print(nn.Sigmoid()(binary_preds))
#     print(ground_truths)
        
#     print(epoch, "val loss:", val_loss, "val lc loss:", val_lc_loss)

    val_auc.append(metrics.roc_auc_score(ground_truths.detach().numpy(), (binary_preds).detach().numpy()))
    val_acc.append(metrics.accuracy_score(ground_truths.detach().numpy(), (binary_preds).detach().numpy().round())) 
    val_f1.append(metrics.f1_score(ground_truths.detach().numpy(), (binary_preds).detach().numpy().round()))
    
    print("Fold:", i)
    
    print("avg auc:", np.mean(val_auc), "std auc:", np.std(val_auc), "stderr auc:", sem(val_auc))
    print("avg acc:", np.mean(val_acc), "std acc:", np.std(val_acc), "stderr acc:", sem(val_acc))
    print("avg f1:", np.mean(val_f1), "std f1:", np.std(val_f1), "stderr f1:", sem(val_f1))
