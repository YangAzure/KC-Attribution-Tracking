...
class lossFunc(nn.Module):
    ...
    def forward(self, pred, labels, scores, problems, practicing_problems, first):
        ...
        labeled_loss = labeled_loss/correct_code
        ...
        return total_loss, binary_preds, ground_truths
...
test1_subjects = pd.unique(label1_df['SubjectID'])
test2_subjects = pd.unique(label2_df['SubjectID'])
train_val_subjects = pd.unique(processed_df[~processed_df['SubjectID'].isin(list(test1_subjects)+list(test2_subjects))]['SubjectID'])
